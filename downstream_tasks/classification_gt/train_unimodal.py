...
from dataloader import prepare_graph_dataloader, prepare_data_unimodal
...
class Trainer:
    def __init__(
        ...
        # define model
        self.model = GraphTransformer(args=args)
    ...
    def train(self):
        ...
        for epoch in range(self.args['num_epochs']):
            ...
            for i, sample_batched in tqdm(enumerate(self.train_dataloader)):
                node_feats_, adjs_, masks_, targets_ = \
                    prepare_data_unimodal(
                ...
                logits_, mc_loss_, ortho_loss_ = self.model(node_feats_, adjs_, masks_)
                loss = mc_loss_ + ortho_loss_
    ...
    def test(self):
        ...
        with torch.no_grad():
            for sample_batched in tqdm(self.test_dataloader):
                ...
                logits_, _, _ = model(node_feats_, adjs_, masks_)
                logits.append(logits_)
                targets.append(targets_)
