...
class Test:
    ...
    def tester(self):
        ...
        prediction_path = f'./predictionreversed'
            ...
            for i, data in tqdm(enumerate(test_dataset)):
                ...
                if self.is_visualize:
                    Image.fromarray(dst_image_fake.astype(np.uint8)).save(
                        f'{prediction_path}/{os.path.basename(dst_image_path)}') #{os.path.basename(src_image_path)}

                print(f'{prediction_path}/{os.path.basename(dst_image_path)}')
