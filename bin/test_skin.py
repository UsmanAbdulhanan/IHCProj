...
from i2iTranslation.dataloaderskin import ImagePatchDataset
from i2iTranslation.models import create_model
from i2iTranslation.postprocess_skin import PostProcess
...
class Test:
    ...
    def test_setup(self):
        ...
        self.args['train.params.save.epoch'] = str(max(model_epochs))
    ...
    def tester(self):
        ...
        prediction_path = f'./predictionforallskin_2/'
        os.makedirs(prediction_path, exist_ok=True)
        model.setup(self.args, epoch=52)
        print('model loaded')
            ...
            for i, data in tqdm(enumerate(test_dataset)):
                ...
                if self.is_visualize:
                    Image.fromarray(dst_image_fake.astype(np.uint8)).save(
                        f'{prediction_path}/{os.path.basename(src_image_path)}') #{os.path.basename(dst_image_path)}
