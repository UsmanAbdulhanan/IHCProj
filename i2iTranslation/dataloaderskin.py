...
class BaseDataset(Dataset):
    def __init__(
        ...
        self.images_path = '/home/comppec/Downloads/VirtualMultiplexer-master/data/skin_images' 
    ...
    def _get_splits(self):
        ...
        self.src_split = os.listdir(f'{self.images_path}/{self.src_marker}')
        self.dst_split = self.src_split.copy()
    def _base_load(self):
        # get src and dst image paths                                                                 
        self.src_image_paths = [f'{self.images_path}/{self.src_marker}/{x}' for x in self.src_split]
        self.dst_image_paths = [f'{self.images_path}/{self.dst_marker}/{x}' for x in self.dst_split]
        ...
        if self.is_train:
            ...
            def _get_patch_paths(marker: str, split: List) -> Dict:
                ...
                for core_name in split:
                    paths_[core_name] = glob.glob(f'{self.i2i_patches_path}/{marker}/{core_name}_*.{self.image_fmt}') # it expects the patch files to have core name init
                return paths_
...
class ImagePatchDataset(BaseDataset):
    ...
    def _load_patches(
        ...
        for path_ in paths:
            ...
            match = re.search(r'_y_(\d+)_x_(\d+)', basename)
            if not match:
                raise ValueError(f"Could not extract x and y from filename: {basename}")
            y = int(match.group(1))
            x = int(match.group(2))
            if self.downsample != 1:
