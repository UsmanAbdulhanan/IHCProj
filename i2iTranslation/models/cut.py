...
class CUTModel(BaseModel):
    ...
    def compute_G_loss(self):
        ...
        # NCE loss: NCE(A, G(A))
        if self.lambda_NCE > 0.0:
            self.loss_NCE_A = self.compute_NCE_loss(self.real_A, self.fake_B) # generator outputs stained (ar)and identity images(real ar to ar)
        else:
            self.loss_NCE_A = 0.0

        # NCE-IDT loss: NCE(B, G(B))
        if self.nce_idt and self.lambda_NCE > 0.0:
            self.loss_NCE_B = self.compute_NCE_loss(self.real_B, self.idt_B) # to learn identity mapping
        else:
            self.loss_NCE_B = 0
