...
class ClassifierROI(nn.Module):
    ...
    def __init__(self, base_filters=16, roi_patch_size=32, num_classes=1):
    ...
    def forward(self, inputs, bboxes):
        ...
        inputs_roi = [] # oh got it thats why in the function np.hstack in train.py to arrange patches with bboxes
        for bbox in bboxes:
            input_idx, y0, x0, y1, x1, cls = bbox
            #find the types of each input  
            roi = inputs[int(input_idx.item()), :, int(y0.item()):int(y1.item()), int(x0.item()):int(x1.item())]
            ...
            # plt.imshow(resized_roi.squeeze(0).detach().permute(1, 2, 0).cpu().numpy())
            # plt.show()
        ...
        outputs_roi = outputs_roi.squeeze()
