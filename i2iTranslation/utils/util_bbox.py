...
def draw_conditions(bboxes, dim, num_classes=2):
    ...
    # plt.imshow(condition[0,0])
    return condition

def bbox_data_generator(patches: np.ndarray, bboxes: List[np.ndarray], nb_rois: int = 8): # we can tune nb_rois to control the number of bboxes per patch
    ...
    for i, bbox in enumerate(bboxes):
        ...
        if bbox.size == 0:
            print(f"Warning: no bbox found for patch {i}, skipping...")
    ...
    return patches, condition_batch, bbox_batch, idx_patches_no_bbox 
